...
export function DiscoverContent({ flyIns, loadError = false }: { flyIns: FlyIn[]; loadError?: boolean }) {
  ...
  return <section className="page-shell discover-page"><div className="page-intro"><p className="eyebrow">DISCOVER THE RAMP</p><h1>Find a reason<br />to <em>fly.</em></h1><p>Browse nearby fly-ins, see the pilots already on the list, and put the next great airport day on your calendar.</p></div><div className="discover-toolbar"><div><p className="eyebrow">NORTH TEXAS · SEEDED AIRPORTS</p><h2>What&apos;s happening on the ramp</h2><p>{displayed.length} public fly-ins available. Event details are live.</p></div></div><div className="filter-row" aria-label="Fly-in category filters"><span>Filter by:</span>{filters.map((item) => <button key={item} className={filter === item ? "selected" : ""} onClick={() => setFilter(item)} aria-pressed={filter === item}>{item}</button>)}</div>{loadError ? <div className="inline-empty"><h3>The ramp report is temporarily unavailable.</h3><p>We could not load fly-ins right now. Please try again shortly.</p></div> : displayed.length === 0 ? <div className="inline-empty"><h3>No fly-ins match this filter yet.</h3><p>Try another category, or create the next reason for pilots to gather.</p></div> : <div className="flyin-grid">{displayed.map((flyIn, index) => <FlyInCard key={flyIn.id} flyIn={flyIn} featured={index === 0} />)}</div>}</section>;
}
