...
export function SiteHeader({ isAuthenticated }: { isAuthenticated: boolean }) {
  ...
  return <header className="topbar"><Link className="brand" href="/" aria-label="The Ramp home"><span className="brand-mark" aria-hidden="true">↗</span>THE RAMP</Link><nav aria-label="Main navigation">{links.map((link) => <Link key={link.href} className={pathname === link.href ? "active" : ""} href={link.href}>{link.label === "Create" ? "+ Create" : link.label}</Link>)}<a href={DISCORD_COMMUNITY_URL} target="_blank" rel="noopener noreferrer" aria-label="Community on Discord (opens in a new tab)">Community</a>{isAuthenticated ? <SignOutButton /> : <><Link className="nav-sign-in" href="/login">Sign In</Link><Link className="nav-join" href="/signup">Join The Ramp</Link></>}</nav>{isAuthenticated ? <Link className="avatar-button" href="/profile" aria-label="Open your pilot profile">ME</Link> : <Link className="avatar-button guest" href="/signup" aria-label="Join The Ramp">↗</Link>}</header>;
}
